...
def _copy_course_metadata_sheet(workbook: Any, metadata_rows: list[tuple[str, Any]]) -> None:
    ws = workbook.create_sheet(COURSE_METADATA_SHEET)
    _write_row(ws, 1, list(COURSE_METADATA_HEADERS), bold=True)
    filtered_rows = [
        (field, value)
        for field, value in metadata_rows
        if normalize(field) != normalize(COURSE_METADATA_FACULTY_NAME_KEY)
    ]
    for idx, (field, value) in enumerate(filtered_rows, start=2):
        _write_row(ws, idx, [field, value], bold=False)
    _apply_sheet_layout_and_protection(
        ws=ws,
        header_row=1,
        header_count=2,
        paper_size=ws.PAPERSIZE_A4,
        orientation=ws.ORIENTATION_PORTRAIT,
    )
...
def _copy_system_hash_sheet(*, source_workbook: Any, target_workbook: Any) -> None:
    if SYSTEM_HASH_SHEET not in source_workbook.sheetnames:
        return
    src = source_workbook[SYSTEM_HASH_SHEET]
    ws = target_workbook.create_sheet(SYSTEM_HASH_SHEET)
    for row in src.iter_rows(values_only=True):
        ws.append(list(row))
    ws.sheet_state = _SHEET_STATE_HIDDEN


def _add_report_integrity_sheet(workbook: Any) -> None:
    protected_sheets = [
        ws.title for ws in workbook.worksheets if ws.title != SYSTEM_REPORT_INTEGRITY_SHEET
    ]
    template_id = ""
    template_hash = ""
    if SYSTEM_HASH_SHEET in workbook.sheetnames:
        system_ws = workbook[SYSTEM_HASH_SHEET]
        template_id_raw = system_ws[_CELL_SYSTEM_TEMPLATE_ID].value
        template_hash_raw = system_ws[_CELL_SYSTEM_TEMPLATE_HASH].value
        template_id = str(template_id_raw).strip() if template_id_raw is not None else ""
        template_hash = str(template_hash_raw).strip() if template_hash_raw is not None else ""
    manifest = {
        ...
        "sheet_order": protected_sheets,
        "sheets": [
            {
                "name": sheet_name,
                "hash": _sheet_fingerprint(workbook[sheet_name]),
            }
    ...
    ws = workbook.create_sheet(SYSTEM_REPORT_INTEGRITY_SHEET)
    ws.append([SYSTEM_REPORT_INTEGRITY_MANIFEST_HEADER, SYSTEM_REPORT_INTEGRITY_HASH_HEADER])
    ws.append([manifest_text, manifest_hash])
    ws.sheet_state = _SHEET_STATE_HIDDEN


def _sheet_fingerprint(worksheet: Any) -> str:
    tokens: list[str] = []
    max_col = worksheet.max_column
    if max_col <= 0 or worksheet.max_row <= 0:
        return sign_payload("")
    col_names = [_excel_col_name(col) for col in range(1, max_col + 1)]
    for row_idx, row in enumerate(
        worksheet.iter_rows(min_row=1, max_row=worksheet.max_row, min_col=1, max_col=max_col, values_only=True),
        start=1,
    ):
        for col_idx, value in enumerate(row, start=1):
            ...
            if token == "":
                continue
            tokens.append(f"{col_names[col_idx - 1]}{row_idx}={value}")
    payload = "\n".join(tokens)
...
def _write_direct_co_sheet(
    *,
    ...
    ws = workbook.create_sheet(f"CO{co_index}{CO_REPORT_DIRECT_SHEET_SUFFIX}")
    report_metadata_rows = _report_metadata_rows(
        metadata_rows,
        co_index=co_index,
        outcome_value_template=CO_REPORT_METADATA_OUTCOME_VALUE_TEMPLATE,
    )
    _write_course_metadata(ws, report_metadata_rows)
    header_row = len(report_metadata_rows) + 2

    active_components = [
        component for component in components if component.max_by_co.get(co_index, 0.0) > 0
    ]
    total_weight = _round2(
        sum(component.weight for component in active_components)
    )
    headers = [
        CO_REPORT_HEADER_SERIAL,
        CO_REPORT_HEADER_REG_NO,
    ...
    for idx, (reg_no, student_name) in enumerate(students, start=1):
        ...
        if absent:
            row_values.extend(
                [CO_REPORT_NOT_APPLICABLE_TOKEN, CO_REPORT_NOT_APPLICABLE_TOKEN, CO_REPORT_NOT_APPLICABLE_TOKEN]
            )
        else:
    ...
    _apply_sheet_layout_and_protection(
        ...
        paper_size=ws.PAPERSIZE_A3,
        orientation=ws.ORIENTATION_LANDSCAPE,
    )


def _write_indirect_co_sheet(
    *,
    workbook: Any,
    co_index: int,
    metadata_rows: list[tuple[str, Any]],
    ...
    ws = workbook.create_sheet(f"CO{co_index}{CO_REPORT_INDIRECT_SHEET_SUFFIX}")
    report_metadata_rows = _report_metadata_rows(
        metadata_rows,
        co_index=co_index,
    ...
    active_components = [
        ...
        if any(
            not (isinstance(value, float) and value == 0.0)
            for value in component.marks_by_co.get(co_index, [])
        )
    ]
    ...
    for component in active_components:
        ...
        headers.append(
            f"{component.name} ({CO_REPORT_SCALED_LABEL_TEMPLATE.format(max_value=scaled_max_value)})"
        )
...
def _write_course_metadata(ws: Any, metadata_rows: list[tuple[str, Any]]) -> None:
    _write_row(ws, 1, list(COURSE_METADATA_HEADERS), bold=True, start_col=2)
    for idx, (field, value) in enumerate(metadata_rows, start=2):
        _write_row(ws, idx, [field, value], bold=False, start_col=2)
...
def _style_cache_for_sheet(ws: Any) -> dict[str, Any]:
    ...
    body_alignment = Alignment(vertical=_ALIGN_CENTER)
    style_cache = {
        ...
        "header_bold": bool(header_style.get(_STYLE_KEY_BOLD, True)),
    }
