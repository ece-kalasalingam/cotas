...
class _DummyModule:
    def __init__(self, step4_path: str | None = None) -> None:
...
def test_step5_blocked_by_dependency_shows_info(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    dummy = _DummyModule()
    dummy._can_run = (False, "instructor.require.step4")
...
def test_step5_missing_source_file_shows_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert critical_calls[0][2] == "instructor.require.step4"
    assert dummy.step5_done is False
...
def test_step5_copy_failure_shows_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    dummy = _DummyModule(step4_path=str(src))
    critical_calls: list[tuple] = []
    ...
    assert len(critical_calls) == 1
    assert critical_calls[0][2] == "app.unexpected_error"
    assert dummy.step5_done is False
...
def test_step5_success_generates_report(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert dummy.step5_done is True
    assert dummy.step5_outdated is False
    assert dummy.step5_path == str(dst)
    assert remembered == [(str(dst), instructor_ui.APP_NAME)]
    assert dummy.status_changed.messages == ["instructor.status.step5_selected"]
